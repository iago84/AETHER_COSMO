...
def periodic_gaussian(x: np.ndarray, y: np.ndarray, t: int, cx: float, cy: float, sigma: float, amplitude: float, dt: float, freq: float) -> np.ndarray:
    g = np.exp(-(((x - cx) ** 2 + (y - cy) ** 2) / (2.0 * sigma ** 2)))
...
def top_hat(x: np.ndarray, y: np.ndarray, t: int, cx: float, cy: float, radius: float, amplitude: float = 1.0, duration: int | None = None) -> np.ndarray:
    r2 = (x - cx) ** 2 + (y - cy) ** 2
...
def lorentzian(x: np.ndarray, y: np.ndarray, t: int, cx: float, cy: float, gamma: float, amplitude: float = 1.0, duration: int | None = None) -> np.ndarray:
    r2 = (x - cx) ** 2 + (y - cy) ** 2
    l = amplitude / (1.0 + (r2 / (gamma ** 2)))
    if duration is not None and t >= duration:
